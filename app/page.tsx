...
function Trend({ store, range }: { store: Store; range: Period }) {
  const points = dates(range).map((date) => {
    ...
    return { date, sales: s.sales.value, spend: s.spend };
  });
  const values = points
  ...
  return (
    <>
      ...
      <div className="chart-scroll">
        <svg
          role="img"
          aria-label="Vendas Shopify e investimento Meta mais Google, numa única escala em euros"
          viewBox="0 0 900 210"
      ...
      <details className="chart-data">
        ...
        <Table
          headers={["Data", "Vendas Shopify", "Meta + Google"]}
          rows={points.map((p) => [
            p.date,
            currency(p.sales),
            currency(p.spend),
      ...
      <div className="main-wrap">
        ...
        <main id="conteudo">
            ...
            {sel.windows.map((w, i) => {
              ...
              return (
                ...
                >
                  ...
                  <strong>{currency(v.sales.value)}</strong>
                  <div className="period-footer">
                    <span>Vendas Shopify</span>
                    <Change a={v.sales.value} b={before.sales.value} />
                  </div>
                </Link>
              );
            })}
          ...
          {section === "overview" ? (
            <>
              <div className="four-col">
                <Kpi label="Vendas totais" m={s.sales} prev={p.sales} />
                <Kpi
                  label="Encomendas"
                  m={s.orders}
                  prev={p.orders}
              ...
              <div className="overview-grid">
                <Panel
                  title="Vendas e investimento"
                  eyebrow="EVOLUÇÃO DO PERÍODO"
                  note="Vendas totais Shopify e custos Meta + Google. Valores em falta não são zero. Totais do período podem ter revisões posteriores às séries diárias."
                >
                ...
                <Panel title="Da visita à compra" eyebrow="LOJA ONLINE">
                  <Funnel s={s} />
                  <p className="panel-note">
                    Sessões Shopify. A taxa resulta de sessões com checkout
                    concluído ÷ sessões.
                  </p>
                </Panel>
              </div>
              <div className="overview-grid">
                ...
                <Panel title="Pulso da operação" eyebrow="PREPARAÇÃO E TRÁFEGO">
                  <div className="pulse-list">
                    {[
                      [
                        "Encomendas preparadas",
                        integer(s.fulfilled.value),
                        "Shopify · fulfillments",
                      ],
                      [
                        "Encomendas expedidas",
                        integer(s.shipped.value),
                        "Estado reportado pela integração",
                      ],
                      [
                        "Utilizadores GA4",
          ...
          <footer>
            ...
            <small>
              Consulta em {localDate()} · Abrir a página não atualiza as fontes.
            </small>
          </footer>
        </main>
      </div>
