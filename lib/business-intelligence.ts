...
function isPaidShopifyRow(r:Record<string,unknown>){return ["PAID","PARTIALLY_REFUNDED"].includes(String(r.order_financial_status||"").toUpperCase());}
...
function isNewsletterAttribution(r:Record<string,unknown>){return /klaviyo|newsletter|e-?mail|email/.test(shopifyAttributionText(r));}
...
export async function getIntelligence(range:Period):Promise<IntelligenceSnapshot>{
  ...
  const dataQuality:DataQualityCheck[]=[
    ...
    {id:"klaviyo",label:"Klaviyo · atribuição auxiliar",status:klaviyoCampaigns.length||klaviyoFlows.length?"warn":"warn",detail:`Klaviyo atribui ${klaviyoRevenue.toFixed(2)} €. É uma leitura de influência/atribuição e pode sobrepor Google, Meta ou Direto; não entra na receita principal nem nos totais por canal.`}
  ];
