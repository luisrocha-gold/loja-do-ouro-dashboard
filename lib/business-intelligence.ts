...
export type IntelligenceSnapshot = { range:Period; ga4:FunnelSnapshot; klaviyoCampaigns:KlaviyoCampaign[]; klaviyoFlows:KlaviyoFlow[]; districtRows:DistrictRow[]; dataQuality:DataQualityCheck[]; recommendations:Recommendation[]; seo:{clicks:number;impressions:number;ctr:number;position:number}; merchant:{active:number;disapproved:number;pending:number} };
...
function sumRows(rows:Record<string,unknown>[],field:string,range:Period,dateField="date"){return rows.reduce((s,r)=>{const d=String(r[dateField]||"").slice(0,10);return inRange(d,range)?s+n(r[field]):s;},0);}
...
export async function getExecutivePulse(reference=new Date()):Promise<ExecutiveWindowSummary[]>{
  ...
  const [shopify,meta,google,ga4,email]=await Promise.all([
    read("shopify",broad,["date","order_financial_status","order_count","order_current_total_price"],SHOPIFY_ACCOUNT),
    ...
    read("klaviyo",broad,["campaign","sent_at","campaign_report_conversions","campaign_report_conversion_value"],KLAVIYO_ACCOUNT),
  ]);
  const paid=shopify.filter(r=>["PAID","PARTIALLY_REFUNDED"].includes(String(r.order_financial_status||"").toUpperCase()));
  const build=(key:ExecutiveWindow,label:string,range:Period,previous:Period):ExecutiveWindowSummary=>{
    const revenue=sumRows(paid,"order_current_total_price",range),prevRevenue=sumRows(paid,"order_current_total_price",previous),orders=sumRows(paid,"order_count",range),prevOrders=sumRows(paid,"order_count",previous);
    ...
    const emailRev=sumRows(email,"campaign_report_conversion_value",range,"sent_at"),prevEmailRev=sumRows(email,"campaign_report_conversion_value",previous,"sent_at"),emailConv=sumRows(email,"campaign_report_conversions",range,"sent_at"),prevEmailConv=sumRows(email,"campaign_report_conversions",previous,"sent_at");
...
export async function getIntelligence(range:Period):Promise<IntelligenceSnapshot>{
  ...
  const paidRevenue=districtRows.reduce((s,r)=>s+r.shopifyRevenue,0),shopifyPaidOrders=districtRows.reduce((s,r)=>s+r.shopifyOrders,0),adSpend=districtRows.reduce((s,r)=>s+r.totalSpend,0),klaviyoRevenue=klaviyoCampaigns.reduce((s,r)=>s+r.conversionValue,0)+klaviyoFlows.reduce((s,r)=>s+r.conversionValue,0),analyticsVariance=paidRevenue?Math.abs(ga4.purchaseRevenue-paidRevenue)/paidRevenue*100:null,purchaseVariance=shopifyPaidOrders?Math.abs(ga4.purchases-shopifyPaidOrders)/shopifyPaidOrders*100:null;
  const dataQuality:DataQualityCheck[]=[{id:"shopify-ga4-revenue",label:"Shopify vs GA4 · receita",status:analyticsVariance===null?"warn":analyticsVariance<=10?"pass":analyticsVariance<=25?"warn":"fail",variancePct:analyticsVariance,detail:analyticsVariance===null?"Sem base para comparação.":`Diferença de ${analyticsVariance.toFixed(1)}% entre receita paga Shopify e purchase_revenue GA4.`},{id:"shopify-ga4-orders",label:"Shopify vs GA4 · compras",status:purchaseVariance===null?"warn":purchaseVariance<=10?"pass":purchaseVariance<=25?"warn":"fail",variancePct:purchaseVariance,detail:purchaseVariance===null?"Sem base para comparação.":`Diferença de ${purchaseVariance.toFixed(1)}% entre encomendas pagas Shopify e ecommerce_purchases GA4.`},{id:"ad-spend",label:"Cobertura de investimento",status:adSpend>0?"pass":"fail",detail:adSpend>0?`Meta + Google com ${adSpend.toFixed(2)} € de investimento no período.`:"Sem investimento devolvido pelas plataformas."},{id:"klaviyo",label:"Klaviyo · atribuição",status:klaviyoCampaigns.length||klaviyoFlows.length?"pass":"warn",detail:`Receita atribuída pelo Klaviyo no período: ${klaviyoRevenue.toFixed(2)} €. É um modelo próprio e não deve ser somado à receita Shopify.`}];
  const recommendations:Recommendation[]=[];const weakDistrict=districtRows.filter(r=>r.totalSpend>=75&&(r.roas??0)<1.5).sort((a,b)=>b.totalSpend-a.totalSpend)[0];if(weakDistrict)recommendations.push({priority:"P0",area:"Geografia",title:`Rever investimento em ${weakDistrict.district}`,rationale:`Investimento ${weakDistrict.totalSpend.toFixed(0)} € com ROAS comercial de ${(weakDistrict.roas??0).toFixed(2)}x e ${weakDistrict.shopifyOrders.toFixed(0)} encomendas pagas.`,action:"Separar campanhas por distrito, reduzir orçamento onde a eficiência comercial é fraca e testar criativos/ofertas locais antes de voltar a escalar.",estimatedImpact:"Redução de desperdício e realocação para zonas com maior retorno."});const highOpp=districtRows.filter(r=>r.opportunityBand==="Alta"&&r.sessions>=200).sort((a,b)=>b.opportunityScore-a.opportunityScore)[0];if(highOpp)recommendations.push({priority:"P1",area:"CRO",title:`Atacar fuga de funil em ${highOpp.district}`,rationale:`${highOpp.sessions.toFixed(0)} sessões, ${highOpp.checkouts.toFixed(0)} checkouts e ${highOpp.analyticsPurchases.toFixed(0)} compras GA4.`,action:"Auditar mobile checkout, confiança, portes, métodos de pagamento e mensagens de recuperação segmentadas por distrito.",estimatedImpact:"Aumento da taxa de checkout→compra sem exigir mais tráfego."});const bestEmail=klaviyoCampaigns[0];if(bestEmail&&bestEmail.conversionValue>0)recommendations.push({priority:"P1",area:"CRM",title:`Replicar aprendizagem de “${bestEmail.campaign}”`,rationale:`Campanha com ${bestEmail.conversionValue.toFixed(0)} € atribuídos, ${bestEmail.conversions.toFixed(0)} conversões e RPR ${bestEmail.revenuePerRecipient.toFixed(2)} €.`,action:"Analisar tema, segmentação, horário, produtos e CTA; criar teste A/B com audiência semelhante e controlo de incrementalidade.",estimatedImpact:"Escalar receita de email sem aumentar investimento em media paga."});if(ga4.checkouts>0&&ga4.purchases/ga4.checkouts<0.25)recommendations.push({priority:"P0",area:"Checkout",title:"Priorizar recuperação e fricção no checkout",rationale:`Apenas ${(ga4.purchases/ga4.checkouts*100).toFixed(1)}% dos checkouts GA4 terminaram em compra no período.`,action:"Cruzar Shopify abandoned checkouts + Klaviyo Checkout Started + GA4 begin_checkout por dia e criar alerta quando a taxa cair abaixo do baseline de 28 dias.",estimatedImpact:"Recuperação de receita já gerada pelo tráfego existente."});
  const seo=searchRows.reduce((a,r)=>({clicks:a.clicks+n(r.clicks),impressions:a.impressions+n(r.impressions),ctr:a.ctr+n(r.ctr),position:a.position+n(r.position)}),{clicks:0,impressions:0,ctr:0,position:0});if(searchRows.length){seo.ctr/=searchRows.length;seo.position/=searchRows.length;}const merchant=merchantRows.reduce((a,r)=>({active:a.active+n(r.product_status_active_count),disapproved:a.disapproved+n(r.product_status_disapproved_count),pending:a.pending+n(r.product_status_pending_count)}),{active:0,disapproved:0,pending:0});
