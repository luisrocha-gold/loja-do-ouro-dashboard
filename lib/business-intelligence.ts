...
export type IntelligenceSnapshot = {
  ...
  abandonment:{ klaviyoAddedToCart:number; klaviyoCheckoutStarted:number; klaviyoPlacedOrders:number; klaviyoRevenue:number };
  seo:{clicks:number;impressions:number;ctr:number;position:number};
  merchant:{active:number;disapproved:number;pending:number;context:string};
};
...
export async function getIntelligence(range:Period):Promise<IntelligenceSnapshot>{
  ...
  const dataQuality:DataQualityCheck[]=[
    ...
    {id:"klaviyo",label:"Klaviyo · atribuição",status:klaviyoCampaigns.length||klaviyoFlows.length?"pass":"warn",detail:`Receita atribuída pelo Klaviyo no período: ${klaviyoRevenue.toFixed(2)} €. É um modelo próprio e não deve ser somado à receita Shopify.`}
  ];
  ...
  const seoBase=searchRows.reduce((a,r)=>({clicks:a.clicks+n(r.clicks),impressions:a.impressions+n(r.impressions),positionWeighted:a.positionWeighted+n(r.position)*n(r.impressions)}),{clicks:0,impressions:0,positionWeighted:0});const seo={clicks:seoBase.clicks,impressions:seoBase.impressions,ctr:seoBase.impressions?seoBase.clicks/seoBase.impressions:0,position:seoBase.impressions?seoBase.positionWeighted/seoBase.impressions:0};
  const ptShopping=merchantRows.filter(r=>String(r.product_status_country||"").toUpperCase()==="PT"&&String(r.product_status_reporting_context||"").toUpperCase()==="SHOPPING_ADS");const merchant=ptShopping.reduce((a,r)=>({active:a.active+n(r.product_status_active_count),disapproved:a.disapproved+n(r.product_status_disapproved_count),pending:a.pending+Math.max(0,n(r.product_status_pending_count)),context:"PT · Shopping Ads"}),{active:0,disapproved:0,pending:0,context:"PT · Shopping Ads"});
  return{range,ga4,klaviyoCampaigns,klaviyoFlows,districtRows,dataQuality,recommendations,abandonment,seo,merchant};
}
